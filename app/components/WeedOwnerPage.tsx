    ...
    <>
      ...
      <main className={styles.main}>
        ...
        <section className={styles.hero}>
          <p className={styles.eyebrow}>{store.hoursLabel} · Adults 19+</p>
          <h1>{store.h1}</h1>
        ...
        <section className={styles.section}>
          <h2>Weed, Cannabis, Flower and Bud — What Are You Looking For?</h2>
          ...
          <div className={styles.termGrid}>
            <article><h3>Weed</h3><p>Weed is everyday language for cannabis and may mean flower, pre-rolls or another cannabis format.</p></article>
        ...
        <section className={styles.visitSection} id="visit">
          <div><p className={styles.kicker}>Open 24 Hours in Etobicoke</p><h2>{store.storeName}</h2><address>{store.streetAddress}<br />{store.city}, ON {store.postalCode}</address></div>
          <div className={styles.visitFacts}><strong>{store.hoursLabel}</strong><a href={`tel:${store.phoneIntl}`}>Phone: {store.phoneDisplay}</a><span>Adults 19+</span></div>

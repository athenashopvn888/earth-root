...
export async function generateMetadata({
  ...
  return {
    ...
    alternates: {
      canonical: `https://earthrootcannabis.ca/item/${slug}`,
    },
    openGraph: {
...
function getJsonLd(item: ItemProduct) {
  ...
  const offers: any = {
    "@type": "Offer",
    url: `https://earthrootcannabis.ca/item/${item.slug}`,
    priceCurrency: "CAD",
  ...
  return {
    ...
    "@type": "Product",
    name: item.name,
    image: item.image ? [item.image.startsWith('http') ? item.image : `https://earthrootcannabis.ca${item.image.startsWith('/') ? '' : '/'}${item.image}`] : undefined,
  ...
  return {
    ...
    "itemListElement": [
      ...
      },
      {
        ...
        "position": 2,
        "name": item.category,
        "item": `https://earthrootcannabis.ca/items/${catSlug}`
      },
      {
        "@type": "ListItem",
        "position": 3,
        "name": item.name,
        "item": `https://earthrootcannabis.ca/item/${item.slug}`
      }
    ]
  };
}

...
export async function generateMetadata({
  ...
  return {
    title: `${item.name} | ${item.category} | EarthRoot Cannabis Etobicoke`,
    description: itemData.metaDescription,
    alternates: {
      canonical: `https://www.earthrootcannabis.ca/item/${slug}`,
    },
    openGraph: {
      title: `${item.name} | EarthRoot Cannabis`,
      description: itemData.metaDescription,
...
function getJsonLd(item: ItemProduct) {
  ...
  const offers: any = {
    "@type": "Offer",
    url: `https://www.earthrootcannabis.ca/item/${item.slug}`,
    priceCurrency: "CAD",
  ...
  return {
    ...
    "@type": "Product",
    name: item.name,
    image: item.image ? [item.image.startsWith('http') ? item.image : `https://www.earthrootcannabis.ca${item.image.startsWith('/') ? '' : '/'}${item.image}`] : undefined,
    ...
    "itemListElement": [
      {
        ...
        "position": 1,
        "name": "Home",
        "item": "https://www.earthrootcannabis.ca"
      },
      {
        "@type": "ListItem",
        "position": 2,
        "name": item.category,
        "item": `https://www.earthrootcannabis.ca/items/${catSlug}`
      },
      {
        "@type": "ListItem",
        "position": 3,
        "name": item.name,
        "item": `https://www.earthrootcannabis.ca/item/${item.slug}`
      }
    ]
  };
}

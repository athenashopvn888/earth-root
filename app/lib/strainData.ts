...
export function getStrainData(
  ...
  const description = `${name} is listed as ${tierDesc} in the ${typeLabel} lane at EarthRoot Cannabis${thcLine}. Compare the current product page, menu notes, weight, and posted price before choosing.`;

  const metaDescription = `${name} at EarthRoot Cannabis in Etobicoke. Compare the current ${tierDesc} listing, product notes, and staff guidance before visiting.`;

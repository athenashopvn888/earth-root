...
import EarthRootWebChat from "./EarthRootWebChat";
...
export default function DeliveryContent() {
  ...
  async function handleEmailSubmit(event: React.FormEvent) {
    ...
    try {
      await fetch(`https://script.google.com/macros/s/AKfycbySrZYxI-NNnXfxY1jXOqHgT2HQi4zst2Fgte6FXTeymat_W_r0o1E3P83EfnVCjEk0/exec?action=delivery_email&email=${encodeURIComponent(email)}&store=ERC01`, { method: "GET", mode: "no-cors" });
    } finally {
  ...
  useEffect(() => {
    ...
    fetch("https://milestone-1-demo.vercel.app/api/catalog?store=ER", { signal: controller.signal })
  ...
  return <main className={styles.main}>
    ...
    <section className={`${styles.hero} ${styles.heroPlain}`}>
      <div><p>EarthRoot Cannabis</p><h1>Weed Delivery in Etobicoke</h1><span>Browse the shared product catalog. The store confirms current availability and delivery details before an order is accepted.</span></div>
    </section>
    <section className={styles.deliveryDetails} aria-label="EarthRoot Cannabis delivery details">
      <strong>$60 PRODUCT MINIMUM</strong>
      <a href="sms:+14375249617"><span>DELIVERY TEXT NUMBER</span> +1 (437) 524-9617</a>
    </section>
    <section className={styles.loyalty} aria-labelledby="loyalty-title">
      ...
      <p className={styles.loyaltyConditions}><strong>Important conditions:</strong> Complimentary items apply only to regular-price Craft or Exotic ounces—not BC Premium. Loyalty prices are firm and cannot be reduced with points. Loyalty-price orders do not include extra complimentary items. The dispatcher confirms current eligibility and any included item before checkout.</p>
    </section>
    <section className={styles.howToOrder} aria-labelledby="how-to-order-title">
      <div><p>HOW TO ORDER</p><h2 id="how-to-order-title">Order with the EarthRoot Cannabis dispatcher</h2><span>LIVE ORDER connects you with the EarthRoot Cannabis dispatcher.</span></div>
      <ol>
        ...
        <li><strong>Confirm with the dispatcher</strong><span>The EarthRoot Cannabis dispatcher confirms availability, delivery details, and next steps.</span></li>
      </ol>
    </section>
      ...
      <div className={styles.catalog}>
        ...
        <div className={styles.grid}>{visible.map((product) => <article className={styles.card} key={product.publicProductId}>
          <button type="button" className={styles.imageButton} onClick={() => setSelected(product)} aria-label={`View details for ${product.name}`}>
            {product.images[0] ? <Image src={product.images[0]} alt={`${product.name} on the EarthRoot Cannabis delivery menu`} fill sizes="(max-width: 640px) 50vw, 280px" unoptimized /> : <span>EarthRoot Cannabis</span>}
          </button>
    ...
    <section className={styles.updates} aria-labelledby="delivery-updates-title">
      <div><p>DELIVERY UPDATES</p><h2 id="delivery-updates-title">Get store delivery updates</h2><span>Enter your email to keep the existing EarthRoot Cannabis delivery notification active.</span></div>
    ...
    <div className={styles.ctaSection}><p>Visit us in-store at <strong>5120 Dundas St W, Etobicoke</strong>. We are <strong>Open 24 Hours</strong>. Call <strong>(647) 382-5122</strong>.</p></div>

...
export default function EarthRootWebChat() {
  ...
  useEffect(() => {
    const initial = window.setTimeout(() => {
      if (token) void refresh(token).catch(() => setNotice("Your previous EarthRoot Cannabis chat could not be restored."));
  ...
  async function start(event: FormEvent) {
    ...
    try {
      const data = await payload(await fetch(`${API_BASE}/api/web-chat/session`, {
        ...
        body: JSON.stringify({ storeId: "ER", customerName: intent === "NEW_CUSTOMER" ? name : "", phone, intent, message: firstMessage, smsConsent }),
      }));
  ...
  return (
    <aside className={`sod-web-chat ${open ? "open" : ""}`} aria-label="EarthRoot Cannabis Web Chat">
      ...
      {open && <section className="sod-chat-panel" role="dialog" aria-modal="true" aria-label="EarthRoot Cannabis Web Chat">
      <header><div><strong>EarthRoot Cannabis Web Chat</strong><small>Start your delivery order with a dispatcher</small></div><button type="button" onClick={() => setOpen(false)} aria-label="Minimize chat">×</button></header>
      ...
      {!token ? (!availability || availability.state !== "AVAILABLE" ? <div className="sod-chat-start sod-chat-paused">
        ...
        <label className="sod-sms-consent"><input required type="checkbox" checked={smsConsent} onChange={(event) => setSmsConsent(event.target.checked)} /><span>I agree to receive one text from EarthRoot Cannabis to confirm this mobile number for my Web Chat. Reply YES to confirm. Message and data rates may apply.</span></label>

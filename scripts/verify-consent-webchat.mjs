...
const chat = await readFile(new URL("../app/delivery/EarthRootWebChat.tsx", import.meta.url), "utf8");
...
assert.ok(delivery.includes("<EarthRootWebChat />"), "Delivery page must render EarthRoot Cannabis Web Chat");
assert.ok(!chat.includes('storeId: "PC"') && !chat.includes("sod-web-chat:PC"), "Reference store identity must not remain");
console.log("EarthRoot Cannabis consent Web Chat contract passed.");
